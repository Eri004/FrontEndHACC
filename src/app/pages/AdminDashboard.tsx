...
// ─── Sidebar ──────────────────────────────────────────────────────────────────
function Sidebar({ page, setPage, open, onClose, onLogout }: { page: Page; setPage: (p: Page) => void; open: boolean; onClose: () => void; onLogout: () => void }) {
  const nav = (p: Page) => { setPage(p); onClose(); };
  return (
    <>
      {open && <div className="fixed inset-0 z-40 bg-black/50 lg:hidden" onClick={onClose} />}
      <aside className={`fixed top-0 left-0 z-50 w-64 h-screen flex flex-col bg-slate-900 transition-transform duration-300 ease-out ${open ? "translate-x-0" : "-translate-x-full"} lg:translate-x-0 lg:sticky lg:top-0 lg:h-screen`}>
        {/* Logo */}
        ...
        {/* Nav - que hace scroll interno si hay muchos items */}
        ...
        <div className="p-4 border-t border-white/[0.06] shrink-0">
          <div className="flex items-center gap-3 px-2">
            ...
            <button onClick={onLogout} className="w-7 h-7 rounded-lg bg-slate-800 flex items-center justify-center text-slate-500 hover:text-slate-200 transition-colors">
...
function Header({ page, onMenu, dark, setDark }: { page: Page; onMenu: () => void; dark: boolean; setDark: (d: boolean) => void }) {
  ...
  return (
    <header className="flex items-center gap-4 px-4 lg:px-6 py-3.5 bg-card border-b border-border sticky top-0 z-30">
      <button onClick={onMenu} className="w-9 h-9 rounded-xl border border-border flex items-center justify-center text-muted-foreground hover:text-foreground transition-colors">
              ...
              <tr key={r.id} className="hover:bg-muted/20 transition-colors group">
                ...
                <td className="px-5 py-3.5">
                  <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    <button className="w-7 h-7 rounded-lg hover:bg-muted flex items-center justify-center text-muted-foreground hover:text-red-600 transition-colors"><Trash2 className="w-3.5 h-3.5" /></button>
                  </div>
                </td>
              </tr>
            ))}
  ...
  return (
    <div className={dark ? "dark" : ""}>
      <div className="flex min-h-screen bg-background">
        <Sidebar page={page} setPage={setPage} open={sidebarOpen} onClose={() => setSidebarOpen(false)} onLogout={onLogout} />
        <div className="flex-1 flex flex-col min-h-screen overflow-y-auto">
          <Header page={page} onMenu={() => setSidebarOpen(true)} dark={dark} setDark={setDark} />
